...
func TestARecordLevelRedefineGeneratesOneTypePerAlternative(t *testing.T) {
	...
	out := t.TempDir()
	if err := generate(io.Discard, descriptor, out, options{packageName: "txn", importPath: "example.com/txn"}); err != nil {
...
func TestThreeAlternativesUnrenamedStillCollide(t *testing.T) {
	...
	err := generate(io.Discard, descriptor, t.TempDir(), options{packageName: "txn", importPath: "example.com/txn"})
	if err == nil {
