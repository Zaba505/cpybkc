...
func (f *filer) emitWriter(b *strings.Builder, walks [][]transition) error {
	...
	line(b, "ordinal int")
	line(b, "")
	line(b, "// raw is the record in hand, laid out before it is emitted: a predicate is")
	line(b, "// evaluated against the bytes that are about to go out, so they exist")
	line(b, "// before the transition is chosen. It is reused between records.")
	line(b, "raw bytes.Buffer")
...
func (f *filer) emitWriteRecords(b *strings.Builder, walks [][]transition, admitted []transition) error {
	for _, record := range admitted {
		...
		line(b, "w.raw.Reset()")
		line(b, "")
		line(b, "cw, err := codec.NewWriter(&w.raw, w.enc)")
		line(b, "if err != nil {")
		line(b, "return fmt.Errorf(\"writing record %%d: %%w\", w.ordinal+1, err)")
		line(b, "}")
		line(b, "")
		line(b, "if err := rec.MarshalCOBOL(cw); err != nil {")
		line(b, "return fmt.Errorf(\"writing record %%d: %%w\", w.ordinal+1, err)")
		line(b, "}")
		line(b, "")
		line(b, "raw := w.raw.Bytes()")
		line(b, "")
...
func (f *filer) emitWriterDiagnostics(b *strings.Builder, walks [][]transition) {
	...
	line(b, "//")
	line(b, "// The record does not belong at this point in the file with the values it has,")
	line(b, "// and the value a predicate tests is the caller's: a writer checks it and")
	line(b, "// reports it when it is wrong rather than deriving one that would satisfy the")
	line(b, "// test and storing it into the field.")
	...
	line(b, "}")
	line(b, "")
	line(b, "return fmt.Errorf(\"record %%d is a %%s and no transition leaving the state the automaton is in admits one whose values satisfy that transition's predicate, so it is reported rather than emitted\", w.ordinal+1, record)")
	line(b, "}")
