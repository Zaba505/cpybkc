...
var (
	_ codec.Unmarshaler = (*BatchHeader)(nil)
	_ codec.Marshaler   = (*BatchHeader)(nil)
	_ codec.Unmarshaler = (*BatchDetail)(nil)
	_ codec.Marshaler   = (*BatchDetail)(nil)
)

// UnmarshalCOBOL reads one BATCH-HEADER out of r, in the order docs/ir/SPEC.md
...
func (x *BatchHeader) UnmarshalCOBOL(r *codec.Reader) error {
	var err error

	if x.HdrType, err = r.ReadAlphanumeric(2); err != nil {
		return fmt.Errorf("BATCH-HEADER: reading HDR-TYPE: %w", err)
	}

	if x.HdrName, err = r.ReadAlphanumeric(18); err != nil {
		return fmt.Errorf("BATCH-HEADER: reading HDR-NAME: %w", err)
	}

	return nil
}

// MarshalCOBOL writes this BATCH-HEADER into w, in the order docs/ir/SPEC.md
...
func (x *BatchHeader) MarshalCOBOL(w *codec.Writer) error {
	var err error

	if err = w.WriteAlphanumeric(x.HdrType, 2); err != nil {
		return fmt.Errorf("BATCH-HEADER: writing HDR-TYPE: %w", err)
	}

	if err = w.WriteAlphanumeric(x.HdrName, 18); err != nil {
		return fmt.Errorf("BATCH-HEADER: writing HDR-NAME: %w", err)
	}

	return nil
}

// UnmarshalCOBOL reads one BATCH-DETAIL out of r, in the order docs/ir/SPEC.md
// resolved its items, and retains the bytes of every slack node it carries and
// of every item the copybook gives no data-name.
//
// It is codec's Unmarshaler. The Encoding is r's: the five axes are properties
// of the file in hand, and Encoding is what this descriptor resolved.
func (x *BatchDetail) UnmarshalCOBOL(r *codec.Reader) error {
	var err error

	if x.DtlAccount, err = r.ReadAlphanumeric(10); err != nil {
		return fmt.Errorf("BATCH-DETAIL: reading DTL-ACCOUNT: %w", err)
	}

	if x.DtlType, err = r.ReadAlphanumeric(2); err != nil {
		return fmt.Errorf("BATCH-DETAIL: reading DTL-TYPE: %w", err)
	}

	if x.DtlMemo, err = r.ReadAlphanumeric(8); err != nil {
		return fmt.Errorf("BATCH-DETAIL: reading DTL-MEMO: %w", err)
	}

	return nil
}

// MarshalCOBOL writes this BATCH-DETAIL into w, in the order docs/ir/SPEC.md
// resolved its items, emitting the bytes retained for every slack node and
// every unnamed item it carries, and zero bytes for one it does not.
//
// It is codec's Marshaler. Two values are the descriptor's rather than the
...
// including the value a discriminator tests — a writer evaluates a predicate
// and never inverts one.
func (x *BatchDetail) MarshalCOBOL(w *codec.Writer) error {
	var err error

	if err = w.WriteAlphanumeric(x.DtlAccount, 10); err != nil {
		return fmt.Errorf("BATCH-DETAIL: writing DTL-ACCOUNT: %w", err)
	}

	if err = w.WriteAlphanumeric(x.DtlType, 2); err != nil {
		return fmt.Errorf("BATCH-DETAIL: writing DTL-TYPE: %w", err)
	}

	if err = w.WriteAlphanumeric(x.DtlMemo, 8); err != nil {
		return fmt.Errorf("BATCH-DETAIL: writing DTL-MEMO: %w", err)
	}
