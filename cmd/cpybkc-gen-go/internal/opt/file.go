...
//
// The input MAY be at its end instead, and that is the file ending rather than
// a record cut short. What the placement gives up is stated where it is
// carried: a final record whose bytes were cut off is indistinguishable from
// one whose delimiter was never written.
...
//
// The final delimiter is emitted rather than chosen about. Two writers left to
// decide produce two different files from one descriptor and one set of
// records, which is the divergence this whole arrangement exists to abolish.
