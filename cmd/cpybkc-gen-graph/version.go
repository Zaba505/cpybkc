...
const (
	...
	// identifies it: docs/plugin/SPEC.md resolves the manifest's `graph` to
	// `cpybkc-gen-graph` on PATH, and nothing inside the executable is
	// consulted.
	//
	...
	//
	// `graph` rather than `mermaid` or `dot`, because the notation is an option
	// of this generator rather than the generator's identity: a manifest naming
	// two generators to draw one automaton twice would be naming the same read
	// of the same descriptor twice.
	pluginName = "cpybkc-gen-graph"
...
// and a build made outside one reports 0.0.0-dev. It is a var and not a const
// because the linker silently ignores a `-X` stamp naming a constant — the
// failure #181 was filed for, where the pipeline appears to be stamping a
// version and the program keeps reporting the tree's.
...
//
// cmd/cpybkc and cmd/cpybkc-gen-go carry the same three lines, and .dagger a
// fourth. They are deliberately not shared: this command imports irpb and the
// standard library and nothing else from this repository, so that the surface
// it exercises is the one an outside plugin author has, and a rule this small
// is cheaper written again than that is worth breaking.
...
// what returning this error does: [run] returns before anything is written, and
// main turns it into an `error:` diagnostic and a non-zero status.
