...
//
// The second of those does not bind yet. The closed sets are still being
// enumerated, and until the first v1.0.0 release one MAY gain a member with
// this left where it is. What that permits, what it requires of the addition
// and what it costs an adopter who pinned an earlier release are
// docs/ir/SPEC.md's "While IR_VERSION_1 is being assembled", which is the one
// statement of it rather than a note per member (#347).
//
...
const (
	...
	// The first release: the version every descriptor this project has written
	// carries, and the one being assembled rather than one anything has been held
	// to yet. Its closed sets are fixed by the first v1.0.0 release, and until
	// then one may still gain a member with this number unchanged. That is a rule
	// with a stated end and a stated cost, not an open licence: docs/ir/SPEC.md,
...
const (
	...
	// is docs/ir/SPEC.md, "While IR_VERSION_1 is being assembled", which permits
	// a closed set still being enumerated to gain a member.
	//
	// It is the shape that rule prices highest — an unset choice a consumer may
	// default rather than a reference it must fail on — so what an old consumer
	// does is stated here, as that rule asks: a consumer that has never heard of
	// this value reads the item as text, translating its bytes through a code
	// page and trimming the trailing ones that match that page's space, and is
	// wrong about both. See also docs/ir/SPEC.md, "An item with no charset
	// carries bytes, not characters".
...
type Arm_Schedule struct {
	...
	// rule asks for it. Such a consumer cannot observe which member of the
	// choice is set: it reads this arm's predicate reference, which a scheduled
	// arm does not set, and gets the zero a scalar with implicit presence
	// defaults to. Zero is an ordinary identifier here and not a sentinel (see
	// Node.id), so what it dereferences is node zero. In a descriptor this
	// project writes that node exists and is the File node, because assemble
	// reserves the root before any record, and the consumer refuses the
	// descriptor as malformed: the node it found is not a Predicate. Where node
	// zero is absent it resolves nothing at all, and cpybkc-gen-go returns
	// unresolved naming the arm's identifier. Either way it finds no Predicate
	// node the identifier names, which is what this position MUST resolve to,
	// and MUST stop. A refusal naming what could not be used is what advancing
	// the version would have bought, and is why the permission is affordable
	// for this addition.
