// The wide sparse Parquet conversion, and its own module for one reason:
// parquet-go must not reach the root go.mod.
//
...
// which is the only enforcement a module boundary has. The precedent is the
// ledger example's conversion, and irpb before it.
module github.com/Zaba505/cpybkc/example/policy/parquet
