...
Nothing here is hand-written except those three inputs, this file, and
[`parquet/`](parquet) — the worked conversion of this extract into the one wide
sparse Parquet table a data platform would query it as, which is a Go module of
its own and is not generated by anything. There is no `roundtrip_test.go` beside
the generated package: that exists in [`ledger/`](../ledger) because a credit
posting's retained slack is unexported and no test outside the package can reach
it, and this example has no slack to reach.
...
in and reviewed, and the generated tree here is already 11,219 lines against
`ledger/`'s 3,014. Twenty-one record types would more than double that again for
nothing the eleven do not already show, both properties being met at nine.

Those two numbers are the tree as it stands after
[#318](https://github.com/Zaba505/cpybkc/issues/318), which is what makes the
doubling above an honest bound rather than a floor. The generated reader and
writer used to carry one predicate function per *state* that tested a predicate
— 93 of them here, with eleven distinct bodies — so that tier alone grew with the
automaton's fan-out rather than with the number of things being discriminated.
One function per distinct `(offset, width, literal)` took roughly 1,200 lines off
this example and left every tier growing linearly in the record count, which is
the growth the paragraph above assumes.

So eleven is the smallest count meeting both, and it is a copybook a person can
