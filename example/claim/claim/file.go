...
func (r *Reader) Next() (Record, error) {
	...
	switch r.state {
	case 0: // the state the descriptor carries as node 30
		// Its one transition, which admits CLAIM-RECORD and carries no predicate: it
		...
		return rec, nil
	case 1: // the state the descriptor carries as node 31
		// Its one transition, which admits CLAIM-RECORD and carries no predicate: it
		// matches every record, and this state offers nowhere else for one to go.
		rec := new(ClaimRecord)
...
func (w *Writer) Write(rec Record) error {
	switch v := rec.(type) {
	case *ClaimRecord:
		return w.writeClaimRecord(v)
	default:
		return fmt.Errorf("record %d: this file's records are CLAIM-RECORD, and a %T is none of them", w.ordinal+1, rec)
	}
}
...
// writeClaimRecord emits one CLAIM-RECORD.
//
...
func (w *Writer) writeClaimRecord(rec *ClaimRecord) error {
