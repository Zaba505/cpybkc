...
func (r *Reader) Next() (Record, error) {
	...
	switch r.state {
	case 0: // the state the descriptor carries as node 24
		// Its one transition, which admits MEMBER-RECORD and carries no predicate: it
		...
		return rec, nil
	case 1: // the state the descriptor carries as node 25
		// Its one transition, which admits MEMBER-RECORD and carries no predicate: it
		// matches every record, and this state offers nowhere else for one to go.
		rec := new(MemberRecord)
...
func (w *Writer) Write(rec Record) error {
	switch v := rec.(type) {
	case *MemberRecord:
		return w.writeMemberRecord(v)
	default:
		return fmt.Errorf("record %d: this file's records are MEMBER-RECORD, and a %T is none of them", w.ordinal+1, rec)
	}
}
...
// writeMemberRecord emits one MEMBER-RECORD.
//
...
func (w *Writer) writeMemberRecord(rec *MemberRecord) error {
